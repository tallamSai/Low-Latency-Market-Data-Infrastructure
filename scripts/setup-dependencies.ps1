...
def setup_external_deps(external_dir: str = "external"):
    ...
    if not json_include.exists():
        ...
        zip_file = external_path / "json.zip"
        if download_file(
            "https://github.com/nlohmann/json/releases/download/v3.11.2/json.hpp",
            str(json_dir / "json.hpp")
        ):
            # Single header - just create the structure
            shutil.copy(json_dir / "json.hpp", json_include / "json.hpp")
            print("nlohmann_json setup complete")
    else:
        print("nlohmann_json already installed")
    
    # Setup moodycamel ConcurrentQueue
    print("\n=== Setting up moodycamel ConcurrentQueue ===")
...
if __name__ == "__main__":
    ...
    setup_external_deps(external_dir)
